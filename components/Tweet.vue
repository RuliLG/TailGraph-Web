<template>
    <div class="bg-white p-4 text-sm hover:bg-gray-100 space-y-4">
        <div class="flex justify-start items-center space-x-4">
            <img :src="image" alt="" class="h-12 w-12 rounded-full object-cover object-center">
            <span class="text-gray-700">
                <strong>{{ name }}</strong>
                <span class="text-gray-500">{{ handle }}</span>
            </span>
        </div>
        <p v-if="text" class="text-gray-700" v-html="text"></p>
        <slot></slot>
